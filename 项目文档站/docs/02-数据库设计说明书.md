...
### 3.21 `biz_rule_config` 规则配置
| 字段 | 类型 | 约束/索引 | 说明 |
|---|---|---|---|
| id | bigint | PK | 规则配置ID |
| rule_group | varchar(50) | uk(rule_group,rule_key,tenant_id) | 分组：seat座位/book图书/credit信用/**task定时任务(开关+cron)** |
| rule_key | varchar(80) | uk | 规则键（checkin_window_min/away_minutes/hold_keep_days/init_score/threshold_blacklist…） |
| rule_value | varchar(100) | NOT NULL | 规则值 |
