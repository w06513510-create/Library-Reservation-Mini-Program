...
| `biz_desk` | 桌子（工位组，三级中间层） | id, area_id, desk_no, capacity, shape, pos_x/pos_y, width/height, rotation |
| `biz_seat` | 座位 | id, area_id, **desk_id**, seat_no, seat_type, **pos_x/pos_y + offset_x/offset_y**, qr_code, status |
...
### 3.3+ `biz_desk` 桌子（工位组，三级空间模型中间层）
> 座位空间从「区域→座位」两级改为「区域 → **桌子** → 座位」三级，让平面图呈现真实自习室「一桌 N 座成组、点空座预约」（参考清华/复旦四人桌）。**桌子存绝对坐标 + 座位存相对桌子偏移**，配桌时座位自动成组排布。

| 字段 | 类型 | 约束/索引 | 说明 |
|---|---|---|---|
| id | bigint | PK | 桌子ID |
| area_id | bigint | idx | 所属区域（biz_area） |
| desk_no | varchar(30) | uk(area_id,desk_no,tenant_id) | 桌号（区域内唯一，如 D01；扫桌面码定位到桌） |
| capacity | tinyint | | 容量：1单人 2双人 4四人 6六人 |
| shape | tinyint | | 桌形：0矩形 1圆 2吧台 |
| pos_x / pos_y | int | | 桌子左上角平面图绝对坐标 |
| width / height | int | | 桌子平面图宽 / 高（px） |
| rotation | int | | 旋转角度（度） |
| sort / status | int / tinyint | | 排序 / 0正常 1停用 |
...
| desk_id | bigint | idx | **所属桌子（biz_desk；三级空间模型新增）** |
| seat_no | varchar(30) | uk(area_id,seat_no,tenant_id) | 座位编号（区域内唯一，如 D01-1） |
| seat_type | tinyint | | 0普通 1靠窗 2沙发 3单间 |
| has_power | tinyint | | 0无 1有 |
| pos_x / pos_y | int | | 平面图绝对坐标（= 桌坐标 + 偏移，冗余便于兼容） |
| offset_x / offset_y | int | | **相对所属桌子左上角的偏移（配桌成组排布）** |
| qr_code | varchar(64) | | 桌面二维码标识（扫码签到） |
| status | tinyint | | 0正常 1停用（实时占用由预约单推导，不落座位表） |

> 约座仍以 **seat 为最小可预约单元**，并发行锁（`SeatMapper.selectForUpdate`）与时段重叠校验不变；`biz_reservation` 冗余 area/floor/venue 由座位推导。平面图接口 `seatStatus` 在每个座位上带出所属桌子几何（桌坐标/宽高/旋转/容量），前端按 `deskId` 成组渲染。演示数据：A区自习重排为 10 张四人桌（2 排×5）× 4 座 = 40 座（`sql/seed_seats_desks.sql`）。
