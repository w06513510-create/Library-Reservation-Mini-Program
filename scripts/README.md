...
> ⚙️ 启动脚本已改名并保留在本目录：`run-backend.bat`（后端）/ `run-frontend.bat`（前端）。**以后启动就用这两个。** 后端脚本为 GBK 编码（`chcp` 仅在启 java 前切一次），对齐宿舍管理项目那份实测可用的写法。

| 脚本 | 用途 | 依赖 / 用法 |
| --- | --- | --- |
| `run-backend.bat` | **后端一键启动（已实测）**：固定 JDK17 + 本地仓库 `D:\Maven\.m2\repository`，用 reactor 打自包含 **fat jar**（`mvn -pl ruoyi-admin -am -Dmaven.test.skip=true package`，不带 `clean` 以避开 flatten 坑）再 `java -jar`——约 20s 起、零下载、效果同 IDEA。脚本自动切到 `..\RuoYi-Vue-Plus`。 | 双击运行。`run-backend.bat`=有 jar 直接起、无则先构建；`run-backend.bat build`=改了后端代码后重构建再起；`run-backend.bat run prod`=第二参数覆盖 profile（默认 dev）。端口 **8199**。**先起 MySQL/Redis（上传还需 MinIO）**。 |
